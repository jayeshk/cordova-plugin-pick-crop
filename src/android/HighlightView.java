...
class HighlightView {
    ...
    private void initStyles(Context context) {
        ...
        context.getTheme().resolveAttribute(fakeR.getId("attr","cropImageStyle"), outValue, true);

        TypedArray attributes = context.obtainStyledAttributes(outValue.resourceId,fakeR.getStyleableArray("CropImageView"));
        try {
            showThirds = attributes.getBoolean(fakeR.getStyleable("CropImageView_showThirds"), false);
            showCircle = attributes.getBoolean(fakeR.getStyleable("CropImageView_showCircle"), false);
            highlightColor = attributes.getColor(fakeR.getStyleable("CropImageView_highlightColor"),
                    DEFAULT_HIGHLIGHT_COLOR);
            handleMode = HandleMode.values()[attributes.getInt(fakeR.getStyleable("CropImageView_showHandles"), 0)];
        } finally {
            attributes.recycle();
        }
    }
